...
describe("blog program", () => {
  ...
  const program = anchor.workspace.Blog as Program;
  const wallet = provider.wallet as anchor.Wallet;

  const findProfilePda = (author: web3.PublicKey) => {
  ...
  const findPostPda = (author: web3.PublicKey, id: number | BN) => {
  ...
  it("creates and updates a post", async () => {
    ...
    // For minimal verification, ensure account still exists
  ...
  it("rejects non-sequential create (InvalidNextPostId)", async () => {
    const [profilePda] = findProfilePda(wallet.publicKey);
    const badPostId = 3; // lastPostId should be 1 now, next allowed is 2
